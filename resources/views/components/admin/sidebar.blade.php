<ul class="space-y-1 text-[15px]">
    ...
    <!-- Nhóm: Ngân hàng câu hỏi -->
    <li 
        x-data="{ open: {{ request()->routeIs('admin.questions.*') ? 'true' : 'false' }} }" 
        class="relative"
    >
        <button 
            @click="open = !open"
            class="flex items-center gap-2 px-2 py-2 rounded-md w-full transition hover:bg-blue-100 
                {{ request()->routeIs('admin.questions.*') ? 'bg-blue-50 font-bold text-blue-700' : '' }}"
        >
        ...
        >
            ...
            <li>
                <a href="{{ route('admin.questions.list') }}"
                   ...
                   {{ request()->routeIs('admin.questions.list') ? 'bg-blue-50 font-bold text-blue-700' : '' }}">
