                ...
                <div class="px-6 py-4">
                    ...
                    @auth
                        @if($latestAttempt && !$latestAttempt->is_completed)
                            <a href="{{ route('exams.continue', $latestAttempt) }}" 
                               class="mt-6 block w-full text-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                Tiếp tục làm bài
                            </a>
                        @else
                            <form action="{{ route('exams.start', $exam) }}" method="POST" class="mt-6">
                                @csrf
                                <button type="submit"
